!function(h, u, c, P) {
    ...
    function ce() {
        h.localStorageAvailable ? (c.setItem("name", Jn.value),
        c.setItem("lang", Qn.value),
        ...
        c.setItem("keyboardlayout", Fe.value),
    ...
    function ge(e, t, n, a) {
        ...
        n.push(o.element),
        E(n, "DOMMouseScroll wheel", function(e) {
            var t;
    ...
    function xe(e, t) {
        ...
        switch (e) {
        case ve:
            we.textContent = I("Something went wrong!"),
            a.querySelector(".message").textContent = t;
            break;
        case be:
            we.textContent = I("Disconnected!"),
            a.querySelector(".message").textContent = t;
            break;
        case m:
            ...
            var o = (W(M).flags & w) == w
              ...
              , r = (i.style.display = t.id == M || r ? "none" : "flex",
            i.querySelector(".button-pair").style.display = M == Pn || o ? "flex" : "none",
            i.querySelector("button.report").style.display = t.reported ? "none" : "",
            qe(t.muted),
            a.querySelector(".report-menu").style.display = "none",
            a.querySelector(".invite").style.display = M == t.id ? "flex" : "none",
            Ce.querySelector(".player"))
              , o = (r.style.display = "",
            R(r),
            ue(t.avatar));
            pe(o, Pn == t.id),
            ...
            break;
        case Se:
            we.textContent = I("Rooms"),
            roomsUpdate(t);
            break;
        case ke:
            we.textContent = 0 == zn ? "Public Room" : "Private Room",
            R(a);
            for (var l = ["Language", "Players", "Drawtime", "Rounds", "Word count", "Hint count", "Word mode", "Custom words only"], s = $("settings"), n = 0; n < Yn.length; n++) {
                ...
                c.appendChild(d),
                c.appendChild(de("span", "name", l[n] + ":")),
                Yn[n]);
                n == ne.CUSTOMWORDSONLY && (d = d ? "Yes" : "No"),
                n == ne.SLOTS && (d = x.length + "/" + d),
                n == ne.LANG && (d = re[d]),
                n == ne.WORDMODE && (d = oe[d]),
                n == ne.DRAWTIME && (d += "s"),
                c.appendChild(de("span", "value", d)),
                s.appendChild(c)
            }
            a.appendChild(s);
            i = u.querySelector("#game-invite").cloneNode(!0);
            E(i.querySelector("#copy-invite"), "click", yo),
            a.appendChild(i);
            break;
        case g:
            we.textContent = I("Settings"),
    ...
    y[ve] = f.querySelector(".modal-container-info"),
    y[be] = f.querySelector(".modal-container-info"),
    y[ke] = f.querySelector(".modal-container-room"),
    y[g] = f.querySelector(".modal-container-settings"),
    E(h, "click", function(e) {
        e.target == f && Me()
    }),
    ...
    function b(e, t, n, a) {
        ...
        u.createElement("span"))
          , e = (a.textContent = t,
        o.appendChild(a),
        Ee.scrollHeight - Ee.scrollTop - Ee.clientHeight <= 20);
        if (Ee.appendChild(o),
        e && Pe(),
        0 < p.chatDeleteQuota)
            for (; Ee.childElementCount > p.chatDeleteQuota; )
                Ee.firstElementChild.remove();
        return o
    }
    var i = void 0
      , Ye = void 0;
    function ze(e) {
        He();
        for (var t = (Ye = e).dataset.tooltip, n = e.dataset.tooltipdir || "N", a = ((i = $("tooltip")).appendChild($("tooltip-arrow")),
        i.appendChild($("tooltip-content", I(t))),
        !1), o = e; o; ) {
    ...
    var Ge = u.querySelector("#select-mobile-keyboard-enabled")
      ...
      , Ke = {
        ...
        init: function(e) {
            ...
            this.columns = 0,
            this.elements.keys = [],
            R(this.elements.rows);
            var t = e.layout
              , i = this;
            function n(e, t, n) {
                var a, o = de("button", "key"), r = "PointerEvent"in h ? "pointerdown" : "click";
                return _e.has(t) ? (a = _e.get(t),
                o.classList.add(a.class),
                o.appendChild(de("span", "material-icons", a.icon)),
                E(o, r, function(e) {
                    a.callback(i)
        ...
        enter: function() {
            0 < this.input.length && (bo(this.input),
            this.input = "",
            this.inputChanged())
        },
        toggleCaps: function() {
    ...
    for (var e = 0; e < Be.length; e++) {
        var Xe = de("option");
        Xe.textContent = Be[e].name,
        Xe.value = Be[e].code,
        Fe.appendChild(Xe)
    }
    E(Fe, "change", function(e) {
        for (var t = void 0, n = 0; n < Be.length; n++)
            Be[n].code == this.value && (t = Be[n]);
        null != t && Ke.init(t)
    }),
    ...
    E(Ke.elements.input, "click", function() {
        Ke.isOpen || (u.documentElement.dataset.mobileKeyboardOpen = "",
        ja(),
        Pe(),
        Ke.isOpen = !0)
    }),
    ...
    function Qe(e, t) {
        if ("children" == t)
            ...
            0 < (o = "placeholder" == t ? e.placeholder : o).length ? Ze.push({
                key: o,
                element: e,
                type: t
    ...
    [])
      , C = (mt(0, {
        ...
        action: function() {
            {
                var e;
                L == M && 0 < St.length && (St.pop(),
                0 < St.length ? (Zt(e = St[St.length - 1]),
                l && l.emit("data", {
                    id: Ya,
                    data: e
                })) : nn())
    ...
    function xt() {
        ...
        if (D.id == j && L == M) {
            if (Lt == nt) {
                ...
                o.stroke();
                a = t / 2,
                e = "url(" + qt.toDataURL() + ")" + a + " " + a + ", default"
            }
        } else
            e = "default";
        C.style.cursor = e
    ...
    var Mt = 0
      ...
      , Dt = 0
      , Et = 0
      , $t = [[255, 255, 255], [0, 0, 0], [193, 193, 193], [80, 80, 80], [239, 19, 11], [116, 11, 7], [255, 113, 0], [194, 56, 0], [255, 228, 0], [232, 162, 0], [0, 204, 0], [0, 70, 25], [0, 255, 145], [0, 120, 93], [0, 178, 255], [0, 86, 158], [35, 31, 211], [14, 8, 101], [163, 0, 186], [85, 0, 105], [223, 105, 167], [135, 53, 84], [255, 172, 142], [204, 119, 77], [160, 82, 45], [99, 48, 13]]
    ...
    for (var Yt = [$("top"), $("bottom")], e = 0; e < $t.length / 2; e++)
        Yt[0].appendChild(Vt(2 * e)),
        Yt[1].appendChild(Vt(2 * e + 1)),
        u.querySelector("#game-toolbar .colors-mobile .top").appendChild(Vt(2 * e)),
        u.querySelector("#game-toolbar .colors-mobile .bottom").appendChild(Vt(2 * e + 1));
    for (e = 0; e < Yt.length; e++)
        pt.appendChild(Yt[e]);
    function zt(e) {
        It = q(e, rt, it);
        ...
        st.querySelector(".size-preview .icon").style.backgroundSize = Nt(It) + "%",
        xt()
    }
    function Ht(e) {
    ...
    function Bt(e, t) {
        Ht(gt[e].element),
        e == Lt && !t || (gt[Mt = Lt].element.classList.remove("selected"),
        gt[e].element.classList.add("selected"),
        Lt = e,
        xt())
    }
    function Gt(e) {
    ...
    function Vt(e) {
        var t = $("color");
        return t.style.backgroundColor = Xt($t[e]),
        t.colorIndex = e,
        t
    }
    function Xt(e) {
    ...
    function Zt(e) {
        ...
        !(M != L && bt < e)) {
            ...
            for (t = kt.length * Jt; t < k.length; t++)
                en(on(k[t]), k[t]);
            vt = Math.min(k.length, vt),
            bt = Math.min(k.length, bt)
        }
    }
    const Jt = 50;
    function Qt() {
    ...
    function tn(e) {
        return (e || 0 < k.length || 0 < St.length || 0 < vt || 0 < bt) && (k = [],
        St = [],
        vt = bt = 0,
        r = Qt(),
        kt = [],
    ...
    function an(e) {
        var t, n, a, o, r, i;
        ((t = e)[0] != et ? t[0] == tt && 0 <= t[2] && t[2] < C.width && 0 <= t[3] && t[3] < C.height : (a = t[3],
        o = t[4],
        r = t[5],
        i = t[6],
        t = Math.ceil(t[2] / 2),
        ...
        (i = {
            ...
            x2: C.width + t + r,
            y2: C.height + t + a
        }).x1 < n && n < i.x2 && i.y1 < o && o < i.y2)) ? (k.push(e),
        M == L && en(on(e))) : console.log("IGNORED COMMAND OUT OF CANVAS BOUNDS")
    }
    function on(e) {
        ...
        case tt:
            ...
            if (s != p[0] || c != p[1] || d != p[2]) {
                ...
                for (var g, f, y, v, b, k, S = u.height, w = u.width; h.length; ) {
                    ...
                    k = b = !1; y++ < S - 1 && m(v); )
                        rn(u, v, s, c, d),
                        0 < f && (m(v - 4) ? b || (h.push([f - 1, y]),
                        b = !0) : b = b && !1),
                        f < w - 1 && (m(v + 4) ? k || (h.push([f + 1, y]),
                        k = !0) : k = k && !1),
                        v += 4 * w
                }
    ...
    function sn() {
        yt.fillStyle = "#FFF",
        yt.fillRect(0, 0, C.width, C.height)
    }
    function cn(e, t, n) {
        ((t = n ? !t : t) ? Gt : Ft)(e)
    }
    ...
    le("Swap", "S", "Swap the primary and secondary color.", Kt),
    E(st.querySelector(".color-picker .preview"), "click", function(e) {
        Kt()
    }),
    ...
    E(u, "keyup", function(e) {
        if ("Enter" == e.code)
            return De.focus(),
            0;
        if ("input" == u.activeElement.tagName.toLowerCase() || "textarea" == u.activeElement.tagName.toLowerCase() || -1 != At)
            return 0;
    ...
    "PointerEvent"in h ? (E("#game-toolbar .colors * .color", "pointerenter", function(e) {
        var t = dn + "\n" + Rt[this.colorIndex];
        u.querySelector("#game-toolbar .colors").dataset.tooltip = t,
        Ue()
    }),
    ...
    E("#game-toolbar .colors-mobile * .color", "pointerdown", function(e) {
        cn(this.colorIndex, 0 == e.button, e.altKey),
        st.querySelector(".colors-mobile").classList.remove("open")
    }),
    E(C, "pointerdown", function(e) {
        if ((0 == e.button || 2 == e.button || 5 == e.button) && -1 == At)
    ...
    E(u, "pointerup", function(e) {
        "touch" == e.pointerType ? un == e.pointerId && (un = null,
        fn(e.button)) : fn(e.button)
    ...
    E(C, "mousedown", function(e) {
        e.preventDefault(),
        0 != e.button && 2 != e.button || -1 != At || gn(e.button, e.clientX, e.clientY, !0, -1)
    }),
    ...
    E(C, "touchstart", function(e) {
        e.preventDefault();
        e = e.changedTouches;
        0 < e.length && null == un && (un = e[0].identitfier,
        gn(0, e[0].clientX, e[0].clientY, !0, e[0].force))
    }),
    ...
    var pn = 0;
    function mn(e, t, n, a) {
        hn(e, t, a = p.pressureSensitivity ? a : -1, n),
        yn(!1)
    }
    ...
    function fn(e) {
        -1 == e || 0 != e && 2 != e && 5 != e || At != e || (pn != k.length && (pn = k.length,
        St.push(pn)),
        un = null,
        At = -1)
    }
    function yn(e) {
        if (D.id == j && L == M && -1 != At) {
            ...
            Lt == nt && (l = It,
            0 <= Ct && (l = (l - rt) * q(Ct, 0, 1) + rt),
            s = Math.ceil(.5 * l),
            c = q(Math.floor(wt[0]), -s, C.width + s),
            e = q(Math.floor(wt[1]), -s, C.height + s),
            r = q(Math.floor(S[0]), -s, C.width + s),
            i = q(Math.floor(S[1]), -s, C.height + s),
            t = t,
            a = l,
            o = c,
            e = e,
    ...
    setInterval(function() {
        var e, t;
        l && D.id == j && L == M && 0 < k.length - vt && (e = vt + 8,
        t = k.slice(vt, e),
        l.emit("data", {
            id: Wa,
            data: t
    ...
    setInterval(function() {
        l && D.id == j && L != M && bt < k.length && (en(on(k[bt]), k[bt]),
        bt++)
    }, 3);
    var vn = u.querySelector("#game-canvas .overlay")
      ...
      , xn = void 0;
    function Mn(e, a, o) {
        var r, i, l = Cn, s = qn, c = e.top - l, d = e.opacity - s;
        Math.abs(c) < .001 && Math.abs(d) < .001 ? o && o() : (r = void 0,
        i = 0,
        xn = h.requestAnimationFrame(function e(t) {
            ...
            bn.style.top = Cn + "%",
            vn.style.opacity = qn,
            i == a ? o && o() : xn = h.requestAnimationFrame(e)
        }))
    }
    function Ln(e) {
    ...
    function Dn(e) {
        for (var t = 0; t < bn.children.length; t++)
            bn.children[t].classList.remove("show");
        switch (e.id) {
        case Q:
            Ln(wn);
            break;
        case V:
            Ln(A),
            A.textContent = I("Round $", e.data + 1);
            break;
        case K:
            Ln(A),
            A.textContent = I("Waiting for players...");
            break;
        case _:
            Ln(A),
            A.textContent = I("Game starting in a few seconds...");
            break;
        case Z:
            Ln(Sn),
            Sn.querySelector("p span.word").textContent = e.data.word,
            Sn.querySelector(".reason").textContent = function(e) {
                switch (e) {
                case U:
            ...
            for (var n = Sn.querySelector(".player-container"), a = (R(n),
            ...
            if (0 < e.data.length) {
                ...
                for (var h = 0; h < 3; h++) {
                    ...
                    if (0 < p.length) {
                        ...
                        v.appendChild($("rank-score", I("$ points", g))),
                        y.appendChild(v),
                        0 == h && l.appendChild($("trophy"));
                        for (o = 0; o < p.length; o++)
                            (k = ue((s = p[o]).player.avatar, 0, 0 == h)).style.left = 15 * -(p.length - 1) + 30 * o + "%",
                            0 == h && (k.classList.add("winner"),
                            k.style.animationDelay = -2.35 * o + "s"),
                            l.appendChild(k)
                    }
                }
                for (var b = Math.min(5, u[3].length), o = 0; o < b; o++) {
                    ...
                    y.appendChild($("rank-score", I("$ points", s.player.score))),
                    d[3].appendChild(y)
                }
                0 < u[0].length ? (D = u[0].map(function(e) {
                    return e.player.name
                }).join(", "),
                T.querySelector(".winner-name").textContent = (0 < u[0].length ? D : "<user left>") + " ",
                T.querySelector(".winner-text").textContent = 1 == u[0].length ? I("is the winner!") : I("are the winners!")) : (T.querySelector(".winner-name").textContent = "",
                T.querySelector(".winner-text").textContent = I("Nobody won!"))
            } else
        ...
        case X:
            if (e.data.words)
                ...
                var D = (s = W(e.data.id)) ? s.name : I("User")
                  , D = (A.textContent = "",
                A.appendChild(de("span", void 0, I("$ is choosing a word!", D))),
                ue(s ? s.avatar : [0, 0, 0, 0], 0, !1));
                D.style.width = "2em",
                D.style.height = "2em",
                A.appendChild(D)
            }
    ...
    Wn.prototype.loadSounds = function() {
        this.addSound(En, "/audio/roundStart.ogg"),
        this.addSound($n, "/audio/roundEndSuccess.ogg"),
        this.addSound(Rn, "/audio/roundEndFailure.ogg"),
        this.addSound(In, "/audio/join.ogg"),
        this.addSound(An, "/audio/leave.ogg"),
        this.addSound(Tn, "/audio/playerGuessed.ogg"),
        this.addSound(Nn, "/audio/tick.ogg")
    }
    ,
    ...
    const w = 4;
    K;
    var l, x = [], M = 0, Pn = -1, L = -1, Yn = [], D = {
        id: -1,
        time: 0,
        data: 0
    }, zn = -1, Hn = 0, Un = void 0, N = new Wn, s = void 0, Bn = !1, Gn = !1, Fn = u.querySelector("#game-wrapper"), ut = u.querySelector("#game-canvas .room"), Kn = u.querySelector("#game-players"), _n = u.querySelector("#game-chat"), Vn = (u.querySelector("#game-board"),
    u.querySelector("#game-bar")), Xn = Kn.querySelector(".players-list"), jn = Kn.querySelector(".players-footer"), Zn = u.querySelector("#game-round"), O = [u.querySelector("#game-word .description"), u.querySelector("#game-word .word"), u.querySelector("#game-word .hints .container")], Jn = u.querySelector("#home .container-name-lang input"), Qn = u.querySelector("#home .container-name-lang select"), ea = u.querySelector("#home .panel .button-play"), ta = u.querySelector("#home .panel .button-create");
    ...
    function ia(t) {
        ...
        a)
            try {
                aiptag && adplayer && void 0 !== adplayer && null !== adplayer && "undefined" !== adplayer ? (ra = t,
                aiptag.cmd.player.push(function() {
    ...
    function la(e, t, n) {
        N.context && N.context.resume && N.context.resume(),
        l && fa();
        var a, o = 0, r = {
        ...
        (l = P(e, r)).on("connect", function() {
            aa(!1),
            l.on("joinerr", function(e) {
                fa(),
                xe(ve, function(e) {
                    switch (e) {
                    case 1:
    ...
    function ua(e) {
        var e = (Hn = e) + 1
          , t = Yn[ne.ROUNDS];
        Zn.textContent = I("Round $ of $", [e, t]),
        Vn.querySelector(".mobile .round span").textContent = e + "/" + t
    }
    function ha() {
    ...
    function pa(e, t) {
        var n, a;
        if (n = D = e,
        null != xn && (h.cancelAnimationFrame(xn),
        xn = void 0),
        n.id == j ? Mn({
            top: -100,
            opacity: 0
        ...
        lo = setInterval(function() {
            ...
            D.id == j && (e = oo),
            D.id == X && (e = ro),
            io.style.animationName = so < e ? so % 2 == 0 ? "rot_left" : "rot_right" : "none",
            so < e && N.playSound(Nn),
            so <= 0 && co()
        }, 1e3),
        Fn.classList.add("toolbar-hidden"),
        xt(),
        ga(!1),
        e.id == Q ? (ha(),
        Fn.classList.add("room")) : Fn.classList.remove("room"),
        e.id == V && (ua(e.data),
        0 == e.data) && ha(),
        e.id == Z) {
            M != L && ka(e.data.word);
            ...
            l ? N.playSound(Rn) : N.playSound($n),
            b(I("The word was '$'", e.data.word), "", v(Ae), !0)
        } else
            e.id != j && (O[0].textContent = I("WAITING"),
        ...
        if (e.id == j) {
            ...
            N.playSound(En),
            tn(!0),
            e.data.drawCommands && (k = e.data.drawCommands),
            b(I("$ is drawing now!", W(L).name), "", v(Ie), !0),
            L != M && _n.querySelector("input").focus(),
            !t)
                for (o = 0; o < x.length; o++)
                    Ja(x[o], !1);
            O[0].classList.remove("waiting"),
            L == M ? (a = e.data.word,
            O[0].textContent = I("DRAW THIS"),
            O[1].style.display = "",
            O[2].style.display = "none",
            O[1].textContent = a,
            Fn.classList.remove("toolbar-hidden"),
            xt()) : (ga(!0),
            va(e.data.word, !1),
            ba(e.data.hints))
        } else {
    ...
    function va(e, t) {
        ...
        o && (n = 3),
        O[0].textContent = I(o ? "WORD HIDDEN" : "GUESS THIS"),
        O[1].style.display = "none",
        O[2].style.display = "",
        R(O[2]),
        O[2].hints = [];
    ...
    function Ba(e) {
        ...
        case wa:
            ...
            a.joinTimeout = setTimeout(()=>{
                b(I("$ joined the room!", a.name), "", v(Ae), !0),
                N.playSound(In),
        ...
        case Ca:
            ...
            }(n.reason, a.name), "", v(Te), !0),
            N.playSound(An)) : (clearTimeout(a.joinTimeout),
            a.joinTimeout = void 0),
            n.id != L || n.reason != ee && n.reason != te || tn(!0));
            break;
        case qa:
            var o = W(n[0])
              , r = W(n[1])
        ...
        case xa:
            ...
            r = _a(o, i).getBoundingClientRect(),
            o = .9 * (r.bottom - r.top),
            i.style.width = o + "px",
            i.style.height = o + "px",
            n.vote ? b(I("$ liked the drawing!", a.name), "", v(Ae), !0) : b(I("$ disliked the drawing!", a.name), "", v(Te), !0));
            break;
        case Na:
            Sa(n);
            break;
    ...
    function Fa(e, t) {
        ...
        Xn.appendChild(n.element),
        me(r, Ga(n)),
        $("player-info"))
          , r = $("player-name", e)
          , e = (n.id == M && r.classList.add("me"),
        o.appendChild(r),
        o.appendChild($("player-rank", "#" + n.rank)),
        o.appendChild($("player-score", I("$ points", n.score))),
        a && o.appendChild($("player-tag", "ADMIN")),
        n.element.appendChild(o),
        E(n.element, "click", function() {
            s = n,
            xe(m, n)
        }),
        $("player-icons"))
    ...
    function Ka(e, t) {
        var n, a, o;
        !e.muted && (o = ((a = W(M)).flags & w) == w,
        n = e.id == L || e.guessed,
        M == L || a.guessed || !n || o) && (a = (e.flags & w) == w,
        o = $e,
        n && (o = Oe),
        a && (o = Te),
        _a(e, $("text", t)),
    ...
    function ja() {
        D.id,
        Q;
        for (var e = getComputedStyle(u.documentElement).getPropertyValue("--PLAYERS_PER_PAGE"), t = (e <= 0 && (t = Math.max(48, Xn.clientHeight),
        e = Math.floor(t / 48)),
        Math.ceil(x.length / e)), n = 0; n < x.length; n++)
            x[n].page = Math.floor(n / e);
        ...
        Xa.element.style.display = 1 < t ? "" : "none"
    }
    function Za() {
        ...
        for (var n, a, o = 1, t = 0; t < e.length; t++) {
            ...
            a = o,
            (n = r).rank = a,
            n.element.querySelector(".player-score").textContent = I("$ points", n.score),
    ...
    for (var to = ut.querySelectorAll('*[id^="item-"]'), no = 0; no < to.length; no++) {
        ...
        to[no].item = ao,
        eo.push(ao),
        E(to[no].item.element, "change", function() {
            var e = this.value;
            "checkbox" == this.type && (e = this.checked ? 1 : 0),
            null != this.item.index && da(this.item.index, e, !0)
        })
    }
    const oo = 10
      , ro = 4;
    ...
    function uo(e) {
        so = e,
        io.textContent = so,
        Vn.querySelector(".mobile .drawtime span").textContent = so + "s"
    }
    var ho, t = u.querySelector("#tutorial"), po = t.querySelectorAll(".page"), mo = ge(t.querySelector(".navigation"), po.length, [t.querySelector(".pages")], function(e, t, n) {
        n && clearInterval(go);
        for (var a = 0; a < po.length; a++)
            po[a].classList.remove("active");
        po[t].classList.add("active")
    }), go = setInterval(function() {
        mo.selected < 4 ? ye(mo, mo.selected + 1, !1) : ye(mo, 0, !1)
    }, 3500), ut = u.querySelector("#game-settings");
    ...
    E(u, "PointerEvent"in h ? "pointerdown" : "click", function(e) {
        Ke.elements.main.contains(e.target) || Ke.isOpen && (delete u.documentElement.dataset.mobileKeyboardOpen,
        ja(),
        Ke.isOpen = !1),
        u.querySelector("#game-toolbar .sizes").contains(e.target) || _t()
    }),
    E([Jn, Qn], "change", ce),
    E(ea, "click", function() {
        var t, e, n, a, o;
        ...
        Bn || (e = "" != t ? "id=" + t : "lang=" + Qn.value,
        Me(),
        aa(!0),
        ia(function() {
            oa(location.origin + "/api/play", e, function(e) {
                aa(!1),
                e.success && la(e.data, t)
            }, !0)
        }))
    }),
    E(ta, "click", function() {
    ...
    E([u.querySelector("#copy-invite"), u.querySelector("#modal-player-button-invite")], "click", yo),
    E(y[m].querySelector("button.kick"), "click", function() {
        Me(),
        null != s && s.id != M && l && l.emit("data", {
            id: 3,
            data: s.id
        })
    }),
    ...
    E(y[m].querySelector("button.votekick"), "click", function() {
        Me(),
        null != s && s.id != M && l && (s.id == Pn ? b(I("You can not votekick the lobby owner!"), "", v(Te), !0) : l.emit("data", {
            id: qa,
            data: s.id
        }))
    }),
    E(y[m].querySelector("button.mute"), "click", function() {
        null != s && s.id != M && (s.muted = !s.muted,
        Va(s, 1, s.muted),
        s.muted ? b(I("You muted '$'!", s.name), "", v(Te), !0) : b(I("You unmuted '$'!", s.name), "", v(Te), !0),
        l && l.emit("data", {
            id: 7,
            data: s.id
        }),
    ...
    E(y[m].querySelector("button#report-send"), "click", function() {
        ...
        0 < e && (null != s && s.id != M && (s.reported = !0,
        l && l.emit("data", {
            id: 6,
            data: {
                id: s.id,
        ...
        b(I("Your report for '$' has been sent!", s.name), "", v(Re), !0)),
        Me())
    }),
    ...
    E(Le, "submit", function(e) {
        return e.preventDefault(),
        De.value && bo(De.value),
        Le.reset(),
        vo(0),
        !1
    }),
    fo(),
    h.localStorageAvailable ? (Jn.value = a("name", ""),
    Qn.value = function(e) {
    ...
    p.displayLang = a("displaylang", "en"),
    p.volume = parseInt(a("volume", 100)),
    p.filterChat = 1 == parseInt(a("filter", 1)) ? 1 : 0,
    p.pressureSensitivity = 1 == parseInt(a("pressure", 1)) ? 1 : 0,
    p.avatar = (t = "ava",
    ut = p.avatar,
    null == (t = c.getItem(t)) ? ut : JSON.parse(t)),
    Ge.value = a("keyboard", 1),
    Fe.value = a("keyboardlayout", "en"),
    Ve(),
    ...
    for (var Co = je[p.displayLang], qo = 0; qo < Ze.length; qo++) {
        var xo = Ze[qo]
          , Mo = Je(Co, xo.key);
        "text" == xo.type && (xo.element.textContent = Mo),
        "placeholder" == xo.type && (xo.element.placeholder = Mo)
    }
    function Lo(e) {
        ho.parts[e].classList.remove("bounce"),
        ho.parts[e].offsetWidth,
        ho.parts[e].classList.add("bounce")
    }
    ...
    ut = (ta = u.querySelector("#home .avatar-customizer")).querySelector(".container"),
    t = ta.querySelectorAll(".arrows.left .arrow"),
    ea = ta.querySelectorAll(".arrows.right .arrow"),
    ta = ta.querySelectorAll(".randomize"),
    (ho = ue(p.avatar)).classList.add("fit"),
    ut.appendChild(ho),
    E(t, "click", function() {
        var e = parseInt(this.dataset.avatarIndex);
        --p.avatar[e],
        p.avatar[e] < 0 && (p.avatar[e] = n[e] - 1),
        Lo(e),
        he(ho, p.avatar),
        ce()
    }),
    E(ea, "click", function() {
        var e = parseInt(this.dataset.avatarIndex);
        p.avatar[e] += 1,
        p.avatar[e] >= n[e] && (p.avatar[e] = 0),
        Lo(e),
        he(ho, p.avatar),
        ce()
    }),
    E(ta, "click", function() {
        p.avatar[0] = Math.floor(Math.random() * n[0]),
        p.avatar[1] = Math.floor(Math.random() * n[1]),
        p.avatar[2] = Math.floor(Math.random() * n[2]),
        Lo(1),
        Lo(2),
        he(ho, p.avatar),
        ce()
    ...
    for (var Do = Math.round(8 * Math.random()), Eo = u.querySelector("#home .logo-big .avatar-container"), $o = 0; $o < 8; $o++) {
        var Ro = [0, 0, 0, -1]
          , Ro = (Ro[0] = $o,
        Ro[1] = Math.round(100 * Math.random()) % z,
        Ro[2] = Math.round(100 * Math.random()) % H,
        100 * Math.random() < 1 && (Ro[3] = Math.floor(20 * Math.random())),
        na && 100 * Math.random() < 35 && (Ro[3] = 96 + Math.floor(4 * Math.random())),
        ue(Ro, 0, Do == $o));
        Ro.index = $o,
        Eo.appendChild(Ro),
        E(Ro, "click", function() {
